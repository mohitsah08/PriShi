...
export function PriShiAutoWorkspacePage() {
  ...
  const isLegacyGuestThreadRoute = Boolean(threadId?.startsWith('guest-'));
  ...
  useEffect(() => {
    if (isLegacyGuestThreadRoute) {
      setCurrentThread(null);
      setPageError('This older guest chat link expired. Start a new chat to continue.');
  ...
  }, [threadId, fetchMessages, isLegacyGuestThreadRoute, navigate, setCurrentThread]);
  ...
  async function handleSend({ message, files, mode = 'chat' }) {
    try {
      setPageError('');
      await sendMessage({
        threadId: activeThreadId?.startsWith('guest-') ? null : activeThreadId,
        message,
        files,
        mode,
        navigate
          ...
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <div className={`text-[18px] font-medium ${dark ? 'text-white' : 'text-black'}`}>
                  PriShi
                </div>
                <div className={`text-[11px] uppercase tracking-[0.18em] ${dark ? 'text-white/45' : 'text-black/35'}`}>
                  AI Routing
                </div>
              </div>
            </div>
