...
export const useChatStore = create((set, get) => ({
  ...
  hydrateForSession(sessionKey) {
    if (!sessionKey || sessionKey === get().activeSessionKey) {
      return;
    }
  ...
  async sendMessage({ threadId, message, files = [], navigate, mode = 'chat' }) {
    const { token } = useAuthStore.getState();
    let activeThreadId = threadId || get().currentThreadId;

    if (typeof activeThreadId === 'string' && activeThreadId.startsWith('guest-')) {
      activeThreadId = null;
      set({
        currentThreadId: null
      });
    ...
    if (mode === 'image') {
      try {
        const { data } = await api.post(
          `/api/images/threads/${activeThreadId}/generate`,
          { prompt: message },
          {
        ...
        const userMessage = normalizeMessage(data.data.userMessage);
        finalMessage = normalizeMessage(data.data.assistantMessage);
        ...
        set((state) => ({
          ...
          messagesByThread: {
            ...state.messagesByThread,
            [activeThreadId]: appendMessages(
              (state.messagesByThread[activeThreadId] || []).filter(
        ...
        set((state) => ({
          streamError: error.response?.data?.message || error.message,
          messagesByThread: {
            ...state.messagesByThread,
            [activeThreadId]: (state.messagesByThread[activeThreadId] || []).map((item) =>
              item.id === tempAssistantId
                ? {
                    ...
                    status: 'failed',
                    content:
                      item.content || error.response?.data?.message || error.message
                  }
                : item
            )
          }
